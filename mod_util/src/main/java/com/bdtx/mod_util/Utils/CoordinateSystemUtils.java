...
public class CoordinateSystemUtils {
     ...
     * 地球坐标系转为其他坐标系
     * @param latLng
     * @param type 0 原始 1 火星 2 百度
     * @return
     */
    public static LatLng castToOtherPoint(LatLng latLng, int type) {
        double[] doubles = new double[]{latLng.latitude, latLng.longitude};
        switch (type) {
            case 0:
                //地球坐标系
                break;
            case 1:
                //返回火星坐标系
                doubles = wgs2GCJ(latLng.latitude, latLng.longitude);
                break;
            case 2:
                //百度坐标系
                doubles = wgs2BD09(latLng.latitude, latLng.longitude);
                break;
        }

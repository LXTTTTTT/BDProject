package com.bdtx.mod_util.View.SlideBackView
...
// 一个挺好用的侧面返回控件，可惜手表根布局不同这里用不了
private val slideBackMap = WeakHashMap<Activity, SlideBack>()  // 使用WeakHashMap防止内存泄漏
...
class SlideBack constructor(private val activity: Activity, private var haveScroll: Boolean = true, private var callBack: () -> Unit) {
    ...
    private var iconViewBackgroundColor = Color.BLACK  // IconView 背景色 默认黑色
    var iconViewHeight: Float = dp2px(160)  // IconView 高度 默认160dp
    var iconViewArrowSize: Float = dp2px(5)  // IconView 箭头图标大小 默认5dp
    var iconViewMaxLength: Float = dp2px(30)  // IconView 最大拉动距离 默认30dp
    var sideSlideLength: Float = iconViewMaxLength / 2  // 侧滑响应距离 默认控件最大宽度/2
    var dragRate: Float = 3f  // 阻尼系数 默认3
